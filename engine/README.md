# orbit-core（CodexOrbit 内置引擎）

> 本目录是 CodexOrbit 随包引擎 `orbit-core.exe` 的完整源码，fork 自上游 [446599/ccodex-rotate](https://github.com/446599/CCODEX-ROTATE)，附带自研补丁：
>
> - 请求级 TTFT（首字节耗时）记录
> - SSE 嗅探上游实际返回的模型（`served_model`）
> - 严格防降智：实得模型 ≠ 请求模型时返回 422 `model_mismatch`
> - 会话锁模型：同一会话内模型漂移自动改回锚定模型
> - 凭据亲和不再劫持手动固定节点（`PinAff`）
> - `/api/strict-model` 运行时开关、`probe_interval_seconds` 等可调参数
> - 启动不再自动弹浏览器面板
>
> 构建：`go build -o orbit-core.exe ./cmd/ccodex-rotate`（Go 1.26+，零第三方依赖）
